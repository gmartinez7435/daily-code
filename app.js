const projects = [
  {
    ...
    description:
      "Build a counter that increases, decreases, and returns to zero.",
    concepts: ["querySelector", "addEventListener", "textContent"],
    mission:
      "Connect all three buttons and never let the count drop below zero.",
  ...
  },
  {
    id: "color",
    title: "Color Mixer",
    description: "Change the page color when a visitor taps a color button.",
    concepts: ["dataset", "forEach", "style"],
    mission:
      "Use one event listener for each button and read its data-color value.",
  ...
  },
  {
    ...
    description: "Calculate a restaurant tip from an amount and a percentage.",
    concepts: ["input", "Number", "template literals"],
    mission: "Show the tip and final total whenever Calculate is pressed.",
  ...
  },
  {
    ...
    description: "Let someone add tasks and remove them when finished.",
    concepts: ["createElement", "append", "remove"],
    mission: "Create a new li for each task and remove it when tapped.",
    html: '<main>\n <h1>Today</h1>\n <div><input id="task" placeholder="Add a task"><button id="add">Add</button></div>\n <ul id="list"></ul>\n</main>',
    css: "body{margin:0;min-height:100vh;display:grid;place-items:start center;background:#f2f4fa;font-family:system-ui;color:#1d2942}main{width:min(85%,340px);margin-top:40px}div{display:flex}input{flex:1;padding:12px;border:1px solid #ccd3e2;border-radius:10px 0 0 10px}button{border:0;background:#526dff;color:white;padding:0 18px;border-radius:0 10px 10px 0}li{background:white;margin:10px 0;padding:14px;border-radius:10px;box-shadow:0 5px 15px #2332}",
    starter:
      "const input = document.querySelector('#task');\nconst addButton = document.querySelector('#add');\nconst list = document.querySelector('#list');\n\n// Add a task when the button is clicked\n",
  },
  {
    id: "quote",
    title: "Quote Shuffler",
    description:
      "Show a random encouraging message whenever the button is pressed.",
    concepts: ["arrays", "Math.random", "textContent"],
    mission:
      "Choose a random item from the quotes array without repeating the UI markup.",
...
function isTodayComplete() {
  return completions.some((item) => item.date === dateKey);
}
...
function renderProject() {
  ...
  $("#concepts").innerHTML = project.concepts
    .map((item) => `<span>${item}</span>`)
    .join("");
  const saved = localStorage.getItem(`cd-code-${project.id}`);
  $("#code-editor").value = saved ?? project.starter;
  updateLines();
  showStarter("html");
  projectPassed = false;
...
function resetHints() {
  ...
  $("#hint-button").textContent = "💡 Give me a hint";
}
function showNextHint() {
  const projectHints = hints[project.id];
  if (hintIndex === projectHints.length - 1) {
    resetHints();
    return;
  }
  hintIndex++;
  $("#hint-box").hidden = false;
  $("#hint-number").textContent =
    `HINT ${hintIndex + 1} OF ${projectHints.length}`;
  $("#hint-text").textContent = projectHints[hintIndex];
  $("#hint-button").textContent =
    hintIndex === projectHints.length - 1
      ? "Hide hints"
      : hintIndex === 0
        ? "Show another hint"
        : "Show final hint";
...
function buildDocument(code, runId) {
  ...
  const validator = validators[project.id].replaceAll(
    "parent.postMessage({",
    `parent.postMessage({runId:${runId},`,
  );
...
function loadPreview(documentText) {
  ...
  previewUrl = URL.createObjectURL(
    new Blob([documentText], { type: "text/html" }),
  );
...
function runCode() {
  ...
  localStorage.setItem(`cd-code-${project.id}`, code);
  $("#save-status").textContent = "Saved";
  $("#console-output").innerHTML = "<span>Running…</span>";
  loadPreview(buildDocument(code, activeRunId));
  setTimeout(() => {
    if ($("#console-output").textContent === "Running…")
      $("#console-output").innerHTML =
        "<span>✓ Code ran. Now test the project in Preview.</span>";
  }, 350);
  switchPanel("preview");
}
...
function updateLines() {
  const count = $("#code-editor").value.split("\n").length;
...
function showStarter(type) {
  document
    ...
    .forEach((b) => b.classList.toggle("active", b.dataset.file === type));
  $("#starter-code").textContent = type === "html" ? project.html : project.css;
}
function updateProgress() {
  ...
  const weekDates = names.map((_, index) => {
    const date = new Date(monday);
    date.setDate(monday.getDate() + index);
    return localDateKey(date);
  });
  const weekly = weekDates.filter((date) => completedDates.has(date)).length;
  $("#progress-text").textContent = `${weekly} of 7`;
  $("#progress-fill").style.width = `${(weekly / 7) * 100}%`;
  $("#streak-count").textContent = weekly;
  $("#week-days").innerHTML = names
    .map((n, i) => {
      const done = completedDates.has(weekDates[i]);
      return `<div class="day ${i === day ? "today" : ""} ${done ? "done" : ""}" aria-label="${["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"][i]}"><i>${done ? "✓" : ""}</i>${n}</div>`;
    })
    .join("");
  ...
  $("#greeting").textContent =
    `Good ${hour < 12 ? "morning" : hour < 18 ? "afternoon" : "evening"}, Gabriel.`;
}
...
document
  .querySelectorAll(".file-tab")
  .forEach((button) =>
    button.addEventListener("click", () => showStarter(button.dataset.file)),
  );
...
$("#reset-code").addEventListener("click", () => {
  if (confirm("Reset your JavaScript for this project?")) {
    activeRunId++;
    $("#code-editor").value = project.starter;
    localStorage.removeItem(`cd-code-${project.id}`);
    projectPassed = false;
    updateCompletionButton();
    $("#console-output").innerHTML =
      "<span>Reset complete. Write code, then Save & Run.</span>";
    loadPreview(buildDocument(project.starter, activeRunId));
    updateLines();
    switchPanel("code");
  }
});
$("#complete-project").addEventListener("click", () => {
  if (!projectPassed || isTodayComplete()) return;
  completions.push({ date: dateKey, projectId: project.id });
  localStorage.setItem("cd-completions", JSON.stringify(completions));
  updateProgress();
  updateCompletionButton();
});
$("#code-editor").addEventListener("input", () => {
  ...
  window.saveTimer = setTimeout(() => {
    localStorage.setItem(`cd-code-${project.id}`, $("#code-editor").value);
    $("#save-status").textContent = "Saved";
  }, 500);
});
$("#code-editor").addEventListener("keydown", (event) => {
  if (event.key === "Tab") {
    ...
    el.value =
      el.value.slice(0, start) + "  " + el.value.slice(el.selectionEnd);
...
window.addEventListener("message", (event) => {
  ...
  row.textContent = (icons[event.data.type] || "› ") + event.data.value;
  if (
    $("#console-output").textContent === "Running…" ||
    $("#console-output").textContent.includes("Code ran.") ||
    $("#console-output").textContent.includes("Reset complete.")
  )
    $("#console-output").innerHTML = "";
  $("#console-output").append(row);
});
updateProgress();
chooseProject();
switchPanel("code");
